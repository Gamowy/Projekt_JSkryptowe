        ...
        for i, filename in enumerate(os.listdir(input_dir)):
            if filename.endswith(".txt"):
                ...
                with open(file, encoding="utf8") as raport:
                    ...
                    section_header = f'''<section><div class="left"><h3>Raport nr.{i+1} <span style="font-weight: normal">(Plik {filename})</span></h3>'''
                    section_table = create_table(board)
                    section_text = f'''</div><div class="right"><div class="score"><span class="kacper">Punkty dla gracza umieszczającego "x"</span>:<br>{score_kacper}</div><div class="score"><span class="olek">Punkty dla gracza umieszczającego "o"</span>:<br>{score_olek}</div><div class="result">{result}</div></div></section>'''
                    print(section_header + section_table + section_text)
